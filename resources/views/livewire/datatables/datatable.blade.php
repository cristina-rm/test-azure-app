<div>
    ...
    <div class="relative">
        <div class="flex justify-between items-center mb-1">
            <div class="flex-grow h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="border border-gray-200 form-input block bg-gray-50 focus:bg-white w-full rounded-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5 focus:outline-none focus:ring focus:ring-red-300 focus:ring-opacity-50 focus:border-red-300" placeholder="{{__('Search')}}" /> {{--placeholder="Search in {{ $this->searchableColumns()->map->label->join(', ') }}"--}}
            ...
            <div class="flex items-center space-x-1">
                ...
                <div x-data="{ init() {
                    ...
                    <button wire:click="export" class="flex items-center space-x-2 px-3 border border-primary rounded-md bg-white text-primary text-xs leading-4 font-medium uppercase tracking-wider hover:bg-gray-100 focus:outline-none"><span>Export</span>
        ...
        <div class="rounded-lg shadow-md bg-white max-w-screen {{--overflow-x-scroll--}}">
                    ...
                    @forelse($this->results as $result)
                        <div class="table-row p-1 divide-x divide-gray-100 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50') }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    <div class="px-6 py-2 whitespace-no-wrap text-sm leading-5 text-gray-900 table-cell align-middle @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif">
                                        {!! $result->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
            ...
            <div class="rounded-lg rounded-t-none max-w-screen rounded-lg py-2 {{--border-b border-gray-200--}} bg-white">
